...
def build_multimodal_model():
    ...
    print(f"[INFO] Mode: Random Forest (Bagging Baseline)")
    ...
    print("[INFO] Step 3/5: GridSearchCV hyperparameter optimization (Random Forest)...")
    ...
    try:
        ...
        for params in tqdm(param_list, desc="Hyperparameter Tuning", ncols=100, colour='green', leave=True):
            temp_model = RandomForestRegressor(random_state=GLOBAL_CONFIG['random_state'], n_jobs=-1, **params)
            # 该参数组合的5折交叉验证
        ...
        model = RandomForestRegressor(random_state=GLOBAL_CONFIG['random_state'], n_jobs=-1, **best_params)
    ...
    try:
        ...
        # 共用时间戳保证模型验证匹配
        model_path = os.path.join(assets_dir, f'model_RF_{version_id}.pkl')
        scaler_path = os.path.join(assets_dir, f'scaler_RF_{version_id}.pkl')
        ...
        print(f"\n[DONE] Training pipeline completed. Random Forest model ready.")
